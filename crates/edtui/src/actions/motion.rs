...
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
pub struct MoveWordForwardStart(pub usize);

impl Execute for MoveWordForwardStart {
    fn execute(&mut self, state: &mut EditorState) {
...
/// Move at the end of the word. Breaks on the first character that is not of
/// the same class as the initial character or breaks on line ending.
/// Furthermore, after the first break, whitespaces are skipped.
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
pub struct MoveWordForwardEnd(pub usize);

impl Execute for MoveWordForwardEnd {
    fn execute(&mut self, state: &mut EditorState) {
        fn move_word_right(state: &mut EditorState) {
            let lines = &state.lines;
            let mut index = state.cursor;
...
mod tests {
    ...
    fn test_move_word_forward() {
        ...
        MoveWordForwardStart(1).execute(&mut state);
