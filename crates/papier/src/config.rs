...
use edtui::{
    actions::{Action as EdTuiAction, Execute},
    input::{key::Key, register::RegisterKey},
    EditorMode, Input,
};
use log::debug;
...
pub struct KeyBindings(pub HashMap<EditorMode, HashMap<Vec<KeyEvent>, EdTuiAction<PapierAction>>>);
...
impl<'de> Deserialize<'de> for KeyBindings {
    ...
    where
        D: Deserializer<'de>,
    {
        let parsed_map = HashMap::<EditorMode, HashMap<String, EdTuiAction<PapierAction>>>::deserialize(deserializer)?;
...
impl From<KeyBindings> for Input<PapierAction> {
    fn from(keybindings: KeyBindings) -> Self {
        let mut input = Input::default();
        for (mode, bindings) in keybindings.0 {
            for (keys, action) in bindings {
                let register_key_fn = match mode {
                    EditorMode::Normal => RegisterKey::n,
                    EditorMode::Insert => RegisterKey::i,
                    EditorMode::Visual => RegisterKey::v,
                    EditorMode::Search => RegisterKey::s,
                    EditorMode::Command => RegisterKey::c,
                };

                let register_key = register_key_fn(keys);
                input.register.insert(register_key, action);
            }
        }
        input
    }
...
#[derive(Clone, Debug, Default, Deref, DerefMut)]
pub struct Styles(pub HashMap<EditorMode, HashMap<String, Style>>);
...
impl<'de> Deserialize<'de> for Styles {
    ...
    where
        D: Deserializer<'de>,
    {
        let parsed_map = HashMap::<EditorMode, HashMap<String, String>>::deserialize(deserializer)?;
